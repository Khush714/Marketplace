 ...
 *
 * PHASE 14 — the fix is resolved from the reference's OWN assignment
 * (getPublicOrder → getRiderLocationForOrder), so the caller can never reach
 * another order's rider. Numeric id attempts are rejected outright.
 *
...
export async function GET(
  ...
  const reference = (await params).id.trim().toUpperCase();
  if (
